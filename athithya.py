...
# Review Model - Added from first document
...
def load_product_data():
    try:
        ...
        # Add IDs to products if they don't already have them
...
# Function to get reviews for products - Added from first document
def get_product_reviews(products):
    product_ids = [product['id'] for product in products]
    reviews = Review.query.filter(Review.product_id.in_(product_ids)).all()
    
    # Create a dictionary to store ratings by product_id
    ratings_by_product = {}
    for review in reviews:
        if review.product_id not in ratings_by_product:
            ratings_by_product[review.product_id] = {
                'total': 0,
                'count': 0,
                'avg': 0
            }
        ratings_by_product[review.product_id]['total'] += review.rating
        ratings_by_product[review.product_id]['count'] += 1
    
    # Calculate average ratings
    for product_id in ratings_by_product:
        if ratings_by_product[product_id]['count'] > 0:
            ratings_by_product[product_id]['avg'] = round(
                ratings_by_product[product_id]['total'] / ratings_by_product[product_id]['count'], 1
            )
    
    # Add ratings to products
    for product in products:
        if product['id'] in ratings_by_product:
            product['avg_rating'] = ratings_by_product[product['id']]['avg']
            product['review_count'] = ratings_by_product[product['id']]['count']
        else:
...
@app.route('/tou')
def tou():
    return render_template('tou.html', current_user=current_user)
@app.route('/security')
def security():
    return render_template('security.html', current_user=current_user)
# Original Products Page - Updated to include reviews
@app.route('/original')
def original_products():
    ...
    products_with_reviews = get_product_reviews(products)
    
    return render_template("original.html", products=products_with_reviews, current_user=current_user)

# Predicted Products Page - Updated to include reviews
@app.route('/predicted')
def predicted_products_page():
    ...
    # Add review data to products
    products_with_reviews = get_product_reviews(predicted_products)
    
    return render_template("duplicate.html", products=products_with_reviews, current_user=current_user)

# Review Routes - Added from first document
